...
const HomePage = () => {
  ...
  const myFavourites = useVisibleCount({ itemWidth: 233, gapX: 24, paddingX: 32, rows: 1 })
  const myPlaylists = useVisibleCount({ itemWidth: 260, gapX: 64, paddingX: 40, rows: 2 })
  ...
  const playlistsUrl = useMemo(() => {
    if (!user?.id) return null;
    return `/api/playlist?userId=${encodeURIComponent(user.id)}`;
  }, [user?.id]);
      ...
      <div className="w-[1625px] mt-[64px] mr-[64px] flex gap-x-[64px]">
        ...
        <div className="w-full flex flex-col flex-1 gap-y-[64px]">
          <div className="w-full flex flex-col gap-[32px]">
            ...
            <div className="w-full text-white px-[20px] py-[16px] grid grid-rows-2 grid-flow-col gap-x-[64px] gap-y-[24px] [grid-auto-columns:260px] rounded-[8px] bg-[#0F0F10]/[10%] border border-white/[80%]">
              <div>
          ...
          <div className="w-full grid grid-rows-1 grid-flow-col justify-between bg-neutral-900">
          ...
          <div className="w-full flex flex-col gap-[32px] text-[white]">
            ...
            {artists.length === 0 ? (
              ...
              <div ref={myFavourites.ref} className="w-full h-[345px] py-[16px] px-[16px] flex-col justify-between grid grid-rows-1 grid-flow-col [grid-auto-columns:233px] rounded-[8px] bg-[#0F0F10]/[10%] border border-white/[80%]">
                {artists.slice(0, myFavourites.visibleCount).map(a => (
